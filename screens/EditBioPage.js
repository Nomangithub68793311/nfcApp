...
function EditBioPage({ navigation, route }) {
  ...
  const fectData = async () => {
    ...
    fetch(`${URL}/photo`, {
      ...
      .catch(err => { console.log(err); navigation.replace('SplashScreen') })
      ...
      <KeyboardAwareScrollView keyboardShouldPersistTaps="handled">
        <ScrollView keyboardShouldPersistTaps="handled" style={{ width: "100%", height: "100%" }} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }}>
          <View style={{ alignItems: 'center', marginBottom: 25, marginTop: windowHeight * .15 }}>
            <Text style={{ textAlign: 'center', fontSize: 20, fontFamily: 'Poppins-SemiBold', color: '#6C6C6C' }} >Please Complete Your Profile </Text>
          </View>
