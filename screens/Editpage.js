...
import {
  Input, Box, Center, NativeBaseProvider,
  Actionsheet,
  useDisclose,
} from "native-base"
...
function Editpage({ handlemodal, handleModelcut, route, navigation }) {
  ...
  const handleModel = () => {
    ...
    if (username.length < 1) {
      ...
      data.append("upload_preset", "nfcApp")
      data.append("cloud_name", "dfcuxshca")
      fetch("https://api.cloudinary.com/v1_1/dfcuxshca/image/upload", {
        method: 'post',
        body: data
      }).then(res => res.json())
        .then(data => {
    ...
    <ImageBackground source={image2} resizeMode="cover" style={styles.image}>
      ...
      <KeyboardAwareScrollView keyboardShouldPersistTaps="handled" >
        ...
        <ScrollView keyboardShouldPersistTaps="handled">
          <IconmAT name="close-outline" size={40} color="#6C6C6C" style={{
          ...
          <View style={styles.btnParentSection}>
            ...
            {loading ? <View style={{ alignItems: 'center', marginTop: 10 }}>
              ...
              <View style={{ alignItems: 'center', marginTop: 10 }}>
                <TouchableOpacity onPress={handleModel} style={{
