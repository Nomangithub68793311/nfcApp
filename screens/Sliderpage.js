...
import { URL } from '../Reducers/action'
...
function Sliderpage({ navigation, item, idd, setToken, setTokenoff, setActivateoffid, setId, setActivateid, setWarn, setUserAfterDelete, setOldtoken, socialid, setSocialid, setOldtokenid, list }) {
  ...
  const SocialDelete = async (soaid) => {
    fetch(`${URL}/deleteSocial/${soaid}/${idd}`, {
      method: "delete",
      headers: {
  ...
  return (
    ...
    <View>
      ...
      <TouchableOpacity style={{ width: viewWidth, margin: 5 }} onPress={onOpen}>
        ...
        <Image style={{ height: imageWidth, width: imageWidth, borderColor: item.active ? 'green' : null, borderWidth: item.active ? 2 : null, borderRadius: 20 }}
          source={item.src} />
      </TouchableOpacity>
      ...
      <Actionsheet isOpen={isOpen} onClose={onClose} size="full" style={{ justifyContent: 'center' }}>
        ...
        <Actionsheet.Content >
          {item.title == "ETH" || item.title == "BTC" || item.title == "USDC" || item.title == "CashApp" || item.title == "Venmo" || item.url == 'sms:' ? null :
